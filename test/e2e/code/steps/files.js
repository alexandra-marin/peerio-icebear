...
Then('Cucumbot can not see the uploaded file in DM', async function() {
    return this.waitFor(
        () =>
            ice.fileStore.getByIdInChat(
                this.uploadedFile.fileId,
                ice.chatStore.directMessages[0].id
            ).deleted
    );
});
...
When('I download the uploaded file', function() {
    ...
    return ice.fileStore
        .getById(this.uploadedFile.fileId)
        .download(name)
        .then(() => {
            this.filesToCleanup.push(name);
        });
...
async function checkIfUploadedFileIsShared(chat) {
    this.cucumbotClient.remoteEval(
        `this.world.uploadedFile={fileId:'${this.uploadedFile.fileId}'}`
    );
...
function cucumbotDownload(chatId) {
    const name = getTempFileName();
    return ice.fileStore
        .getByIdInChat(this.uploadedFile.fileId, chatId)
        .download(name)
        .then(() => {
            this.filesToCleanup.push(name);
        });
...
Then('Cucumbot can not download the uploaded file', function() {
    return ice.socket
        .send('/auth/file/url', { fileId: this.uploadedFile.fileId })
        .timeout(5000)
        .then(resp => {
