...
describe('Crypto Keys module', function() {
    ...
    describe('account keys', () => {
        ...
        it('cannot be derived without the right salt', () => {
            return crypto
                .deriveAccountKeys(this.username, this.passphrase, util.b64ToBytes('nonsense'))
                .then(actual => {
    ...
    describe('ghost/ephemeral keys', () => {
        ...
        it('can be derived', () => {
            return crypto.deriveEphemeralKeys(this.ghostID, this.passphrase).then(kp => {
        ...
        it('cannot be derived from a bad id', () => {
            return crypto
                .deriveEphemeralKeys('not the real ghost id heheheheh', this.passphrase)
                .then(kp => {
        ...
        it('cannot be derived from a bad passphrase', () => {
            return crypto.deriveEphemeralKeys(this.ghostID, 'blabla bla bla').then(kp => {
                expect(kp.secretKey).not.to.deep.equal(this.expected.secretKey);
                expect(kp.publicKey).not.to.deep.equal(this.expected.publicKey);
            });
        });
