...
describe('Crypto Keys module', function() {
    ...
    describe('account keys', () => {
        before(() => {
            ...
            this.salt = util.b64ToBytes(
                'r/g3Xm1OSaESajdXKPjxsefpjH7cKgSyX14KRUtepw0='
            );

            this.expected = {
                bootKey: util.b64ToBytes(
                    '6VdJvLy8r/bDf9iXNIHhktuf4j20IqixWjnX57cQ0QU='
                ),
                authKeyPair: {
                    publicKey: util.b64ToBytes(
                        '5clLsoQ9M53zkq9L6SJn01HtlDLPeUdz4ic5GjIsWEI='
                    ),
                    secretKey: util.b64ToBytes(
                        'SfhzTkRcLxw12REVkPDBntIE9sSFn/WMoNAmCOrS8RA='
                    )
                }
            };
        });
        ...
        it('can be derived', () => {
            return crypto
                .deriveAccountKeys(this.username, this.passphrase, this.salt)
                .then(actual => {
        ...
        it('cannot be derived without the right passphrase', () => {
            return crypto
                .deriveAccountKeys(
                    this.username,
                    'not hidden award watts chained restored',
                    this.salt
                )
                .then(actual => {
                    expect(actual).not.to.deep.equal(this.expected);
        ...
        it('cannot be derived without the wrong username', () => {
            return crypto
                .deriveAccountKeys('badusername', this.passphrase, this.salt)
                .then(actual => {
                    expect(actual).not.to.deep.equal(this.expected);
                });
        });
    ...
    describe('ghost/ephemeral keys', () => {
        before(() => {
            this.ghostID = util.strToBytes(
                'CvfX223vsFuVerNrGS1n1sz4AYfpERb8JbeBeWUYMqdo'
            );
            this.passphrase = 'latch floats varied harper vast';

            this.expected = {
                publicKey: util.b64ToBytes(
                    'QOwX70IlZEFh0SlNywooVYiA5OAHkIWn2xa2hoTi5xg='
                ),
                secretKey: util.b64ToBytes(
                    'u7CN24qe9AwUh6Hzu6/J2FR9UnVOKfEw7X7p3lw4vzE='
                )
            };
        });
        ...
        it('can be derived', () => {
            return crypto
                .deriveEphemeralKeys(this.ghostID, this.passphrase)
                .then(kp => {
        ...
        it('cannot be derived from a bad id', () => {
            return crypto
                .deriveEphemeralKeys(
                    'not the real ghost id heheheheh',
                    this.passphrase
                )
                .then(kp => {
                    expect(kp.secretKey).not.to.deep.equal(
                        this.expected.secretKey
                    );
                    expect(kp.publicKey).not.to.deep.equal(
                        this.expected.publicKey
                    );
                });
        });
        ...
        it('cannot be derived from a bad passphrase', () => {
            return crypto
                .deriveEphemeralKeys(this.ghostID, 'blabla bla bla')
                .then(kp => {
                    expect(kp.secretKey).not.to.deep.equal(
                        this.expected.secretKey
                    );
